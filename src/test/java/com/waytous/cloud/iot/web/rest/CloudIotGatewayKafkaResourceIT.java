package com.waytous.cloud.iot.web.rest;
...
import com.waytous.cloud.iot.config.KafkaProperties;
...
class CloudIotGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        CloudIotGatewayKafkaResource kafkaResource = new CloudIotGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/cloud-iot-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cloud-iot-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
